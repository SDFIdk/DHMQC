...
parser=ArgumentParser(description="Generate DSM for a las file. Will try to read surrounding tiles for buffer.",prog=progname)
...
def main(args):
	...
	surfacename=os.path.join(pargs.output_dir,"dsm_"+basisname+".tif")
	#There can be e,g. a "pre" in front of the tilename - get that
	prename="" 
	i=basisname.find(kmname)
	if i>0:
		prename=basisname[:i]
	if os.path.exists(surfacename):
		print(surfacename+" already exists... exiting...")
		return 1
	...
	for j in range(-1, 2):
		for i in range(-1, 2): 
			...
			if os.path.exists(aktFnam):
				...
				pcA[(i,j)]=pointcloud.fromLAS(aktFnam,include_return_number=True).cut_to_return_number(1).cut_to_box(*extent_buf).cut_to_class(cut_surface)
			else:
	...
	if bufpc.get_size()>3:
		...
		if (g.grid==ND_VAL).all():
			return 3
		del bufpc
		g.save(surfacename, dco=["TILED=YES","COMPRESS=DEFLATE","PREDICTOR=3","ZLEVEL=9"],srs=SRS_WKT)
