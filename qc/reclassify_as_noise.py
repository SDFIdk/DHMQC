...
parser=ArgumentParser(description="Use haystack to reclassify some points (levitating plants / birdies) as high noise.",prog=progname)
parser.add_argument("-olaz",help="Output as laz (will store a temporary file and translate that)",action="store_true")
parser.add_argument("las_file",help="input 1km las tile.")
parser.add_argument("badpts_file",help="Binary file containing doubles in the order: x,y,z,c,pid")
...
def main(args):
	...
	exclude_size=os.path.getsize(pargs.badpts_file)
	if exlude_size==0:
		return 1
	...
	cmd=[HAYSTACK,"-o",outname,"-w",pargs.badpts_file,pargs.las_file]
