...
Primarily we want to find points which lie in a vegetation class, but should NOT be included in a
DSM e.g. points on wires - in general points which are floating in the air without much around them.
We can also easily find wrongly classified vegetation points on roofs, walls etc.
In order to find wires with sufficiently dense point distribution to be actually connected to the
ground component, we need to do some morphology.
...
parser.add_argument(
    ...
    type=float,
    default=adddsm,
    help='''Add this amount to generated dsm in order to exclude som floating trees.
            Default: {0:.2f}'''.format(adddsm))
...
def usage():
    '''
    Allows qc_wrap.py to display the help text for this script.
    '''
    parser.print_help()
...
def main(args):
    '''
    Voxelize pointcloud and find voxels floating above the ground.
    '''
    try:
    ...
    # problem - there will be more vegetation floating around then - some real
    # and some wrongly classified (noisy stuff on roofs etc).
    ...
    # floating in air, above ground component - not really close to building
    # (along walls or on top of roof) -and has 'simple' geometry
    ...
    array_geometry.lib.find_floating_voxels(
        labels,
        F,
        gcomp,
        labels.shape[0],
