...
from qc.thatsDEM import pointcloud, array_geometry, triangle
...
sl = "*-*" * 23
pl = "+" * (len(sl))
# hmm not pretty right now - will construct a sequence of args from files
# and args (can put everything in args if we dont need the
# os.path.exists...)...
UNIT_TESTS = [
...
TESTS = [
    ...
    ("roof_ridge_strip", {"files": [LAS_DEMO, BUILDING_DEMO],
                          "args":["-search_factor", "1.1", "-use_all"]}),
    ...
    ("roof_ridge_alignment", {"files": [LAS_DEMO, BUILDING_DEMO],
                              "args":["-use_all", "-search_factor", "1.1"]}),
...
def main(args):
    ...
    if not os.path.exists(OUTDIR):
        os.mkdir(OUTDIR)
    logname = os.path.join(OUTDIR, "autotest_" +
                           "_".join(time.asctime().split()).replace(":", "_") + ".log")
            ...
            for root, dirs, files in os.walk(C_SOURCE_FOLDER):
                for name in files:
                    if name.endswith(".h") or name.endswith(".c"):
                        source_mod_time = max(
                            source_mod_time, os.path.getmtime(os.path.join(root, name)))
            ...
            if pargs.test is None or pargs.test == test:
                ...
                if test in loaded_tests:
                    n_serious += run_test(test, loaded_tests[test],
                                          test_data["files"], stdout, stderr, test_data["args"])
                else:
