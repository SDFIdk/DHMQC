...
import numpy as np
import glob
from qc.thatsDEM import pointcloud, array_geometry,grid
from qc import dhmqc_constants
...
#a simple logger object which other modules can use instead of 'print'
class Logger(object):
	def __init__(self,win,signal):
		self.win=win
		self.signal=signal
	def log(self,text):
		self.win.emit(self.signal,text)
...
class PcPlot_dialog(QtGui.QDialog,Ui_Dialog):
	def __init__(self,iface): 
		...
		self.lbl_path_field.setText("'"+self.index_path_field+"'")
		self.buf_dist=None
		self.n_temp_lines=0
		self.n_temp_polys=0
		self.index_layer_id=None
		...
		self.logger=Logger(self,self.log_signal)  #for use by other modules...
		tile_coverage.LOGGER=self.logger  #make tile_coverage log to this window...
	...
	def buildIndexLayer(self):
		...
		if len(las_path)==0:
			...
			return
		f_name=unicode(QFileDialog.getSaveFileName(self, "Select an output file name for las index:",self.dir,"*.sqlite"))
		if f_name is None or len(f_name)==0:
			return
		self.dir=las_path
		if self.chb_walk_folders.isChecked():
			depth=None
		else:
			depth=1
		self.runInBackground(self.buildIndexLayerInBackground,self.finishIndexLayerTask,(las_path,f_name,depth))
		
	def buildIndexLayerInBackground(self,las_path,f_name,depth=None):
		#to be run in da background...
		self.index_file_name=None
		try:
			con,cur=tile_coverage.connect_db(f_name,False)
			self.logLater("So far so good","yellow")
		except Exception,e:
			...
			try: #always escape and emit signal if something goes wrong...
				tile_coverage.append_tiles(con,cur,las_path,[".las",".laz"],wdepth=depth,rexclude=None)
			except Exception,e:
				self.logLater("An exception occurred {0:s}".format(str(e)),"red")
			else:
				self.index_file_name=f_name #set the attr to signal that all is well.
			cur.close()
			con.close()
		self.emit(self.background_task_signal)
		
	def finishIndexLayerTask(self):
		#called when index layer building has finished
		if self.index_file_name is not None:
			index_layer=QgsVectorLayer(self.index_file_name,"las_index","ogr")
			QgsMapLayerRegistry.instance().addMapLayer(index_layer)
			self.setIndexLayer(index_layer)
	...
	def setIndexLayer(self,index_layer):
		self.index_layer_id=index_layer.id()
		index_layer_name=index_layer.name()
		if id!=self.index_layer_id:
			self.pc=None
			self.pc_in_poly=None
			self.poly_array=None
			self.line_array=None
			self.log("New index layer selected... attributes cleared","blue")
		self.lbl_index_layer.setText(index_layer_name)
		fm=index_layer.dataProvider().fieldNameMap()
	...
	def getIndexLayer(self):
		if self.index_layer_id is None:
			self.log("Select an index layer first!","red")
			return None
		return self.getVectorLayer(self.index_layer_id)
		
	@pyqtSignature('')
	def on_bt_select_index_layer_clicked(self):
		layers,ids=self.getVectorLayerNames(QGis.Polygon)
		keys=[layer+",id="+id for layer,id in zip(layers,ids)]
		if len(layers)>0:
			key,ok=QInputDialog.getItem(self,"Select an index layer (polygons)","Layers with ids:",keys,editable=False)
			if ok:
				i=key.rfind("id=")
				id=key[i+3:]
				index_layer=self.getVectorLayer(id)
	...
	def on_bt_set_index_path_field_clicked(self):
		index_layer=self.getIndexLayer()
		...
		if ok:
			...
			self.lbl_path_field.setText("'"+key+"'")
	@pyqtSignature('')
	...
	def getPointcloudAndVectors(self,dim,method_on_finish):
		...
		layer=self.getVectorLayer(layer_id)
		if layer is None:
		...
		if (self.pc_in_poly is None) or (self.poly_array is None) or (not self.polysEqual(arr,self.poly_array)):
			...
			self.runInBackground(self.loadPointcloud,method_on_finish,(geom.exportToWkt(),self.poly_array,index_layer))
